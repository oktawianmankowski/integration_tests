...
public class BlogApiTest {
    ...
    public void postBlogUserShouldResponseWithStatusCreatedAndNewUserId() throws Exception {
        Mockito.when(blogService.createUser(user)).thenReturn(userId);
        ...
        mvc.perform(post("/blog/user").contentType(MediaType.APPLICATION_JSON_UTF8)
                ...
                .andExpect(content().string(writeJson(new Id(userId))));
    }

    @Test
    public void dataIntegrityViolationExceptionGeneratesConflict() throws Exception {
        Mockito.when(blogService.createUser(user)).thenThrow(new DataIntegrityViolationException("TEST"));
        String content = writeJson(user);
        mvc.perform(post("/blog/user").contentType(MediaType.APPLICATION_JSON_UTF8)
    ...
    @Test
    public void requestForMissingUserReturnsIsNotFound() throws Exception {
